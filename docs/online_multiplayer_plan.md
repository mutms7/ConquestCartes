...
transport with a WebSocket relay for browser play:

1. Godot host creates an online room.
2. Vercel relay assigns a 4-letter lobby code.
...
- `api/relay.js` is a Vercel WebSocket relay with in-memory 4-letter rooms.
- `scripts/ui/main_ui.gd` now has an online WebSocket transport alongside the
...
prototype relay because Vercel can move new WebSocket connections to different
function instances and WebSocket functions have maximum durations.
...
4-letter uppercase lobby codes, WebSocket transport, player assignment, host
...
across function instances with Redis/pubsub and handle WebSocket maximum
duration reconnects. If moving to Durable Objects or PartyKit, keep the same
