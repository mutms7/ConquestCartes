...
This document prepares a future, user-supplied kingdom catalog. It intentionally
adds no cards to the playable pool and does not change the current market.

## Originality and source-list checkpoint

- Every imported card must receive a new Conquest Cartes name, description,
  illustration identity, and stable snake-case `id`. Do not copy a commercial
  card name, exact rules sentence, recognizable terminology, or artwork.
- Treat the requested reference expansion as research input only. Before any
  import, manually verify the source list and record that removed entries were
  excluded; record only this checkpoint, never the source card names or text.
- Preserve the project's voice and effect ordering in
  `docs/card_wording_conventions.md`; mechanical inspiration is not player-facing
  copy.

## Catalog shape and art

Add user-authored definitions to `data/cards/kingdom_catalog.json` only after
review. The empty `cards` array is deliberate. Each entry should follow the
fields and `special_effects` contract in `docs/card_design_rules.md` and should
remain independent of UI or rules code.
...
Until those steps are complete, leave `cards` empty and keep the current game
behavior unchanged.
